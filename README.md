...
- **State recovery** - treehouse writes pool state atomically via a temp file and replacement.
  If an existing state file is empty, truncated, or missing an on-disk worktree, treehouse rebuilds the missing entries and marks them leased until you verify them with `treehouse status`.
...
Commit a `.worktreeinclude` file to seed selected gitignored files from the main checkout into each acquired worktree. This is useful for local configuration or generated files that every worktree needs but Git should not track.
