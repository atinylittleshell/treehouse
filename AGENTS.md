...
- `WriteState` is atomic: it writes to a temp file in the pool directory, fsyncs it, commits it with the platform replacement primitive, and syncs the parent directory where supported.
  ...
  Recovery cannot reconstruct the trusted inventory of seeded ignored files, so `return` refuses recovered entries; after inspection, remove one with an exact-path `destroy --include-leased`
