...
- The worktree's own directory name (the leaf of `<pool>/<slot>/<repo>`) is opt-in unique: `unique_leaf` in `treehouse.toml`, `TREEHOUSE_UNIQUE_LEAF`, and `treehouse get --unique-leaf`, resolved by `config.ResolveUniqueLeaf` in the same precedence as `--root`. The flag argument is a `*bool` because `false` is a bool's zero value AND a meaningful choice: only `cmd.Flags().Changed("unique-leaf")` distinguishes "not typed" from `--unique-leaf=false`, which is what lets one invocation opt out of a pool that enables it in config; an unset or unparseable env var falls through to config for the same reason. It is read ONLY on the creation branch of `pool.acquire` (leaf becomes `repoName + "-" + name`); the reuse loop hands back the path already in state, so enabling it never moves, renames, or invalidates an existing slot and both layouts coexist in one pool. Nothing else may derive a worktree path from the repository name — `internal/pool/state.go` is the sole authority for where a slot lives, and prune/destroy/status/lease all read it
