...
- `get --lease` (see `getLeaseRunE`) is the non-interactive acquire: it opens no subshell, routes hook output and banners to stderr, and keeps path-only stdout unchanged. `get --lease --json` returns `pool.AcquireLeaseInfo`, and `status --json` exposes the same `lease_id`, holder, and timestamp. Conditional return uses `pool.ReleaseConditional` with `--if-lease-id` and optional `--if-lease-holder`; comparison, caller-side preparation, reset, and final clear share one `WithStateLock`. `--guarded` instead uses `pool.ReleaseGuarded`: it requires an exact lease identity, excludes its caller process ancestry from the in-worktree process scan, checks unrelated process and dirty state plus durable `HEAD` reachability under the lock, fails closed while preserving the lease, and on success writes a lease-specific `refs/treehouse/guarded/` recovery ref before clearing lease metadata without terminating processes or resetting the worktree. Return without conditions keeps the legacy path-only behavior
