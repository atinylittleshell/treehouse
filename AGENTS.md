...
- Every command bounds its own blocking. `internal/deadline` holds ONE process-wide budget (`--timeout`, then `TREEHOUSE_TIMEOUT`, then `deadline.Default` = 10m; `0` waits forever), armed once in `rootCmd.PersistentPreRun`. Exactly three places consult it, because they are the only places treehouse blocks: `pool.WithStateLock` (polls `tryLockFile` every `lockPollInterval` instead of a blocking `flock(LOCK_EX)`, which cannot be cancelled), and the two subprocess chokepoints `gitvcs.runGitRaw` / `jjvcs.runJJ` (`exec.CommandContext`, which ALSO kills the child - without it, killing treehouse leaves `git fetch` reparented to init). The budget is deliberately process-wide rather than a `context.Context` threaded through `vcs.Backend`: one process runs one command, and the interface has 19 operations whose only context consumer would be those three sites. It is per PHASE, not per process - `get` calls `deadline.Restart()` after its subshell exits, because the subshell is deliberately unbounded and would otherwise leave the return path judged against an expired budget. Lifecycle hooks are likewise unbounded by design. `WithStateLock` stamps the holder pid into the lock file (best effort, diagnostic only) and says who it is waiting on after `lockNoticeAfter`, so a contended pool is distinguishable from a wedged one
