...
- Acquire reuses a slot only when it is idle, unleased, clean, and HEAD is merged into that exact reset target (resolved once to an immutable commit so the guard and `ResetWorktreeToRef` cannot disagree); it fails closed when dirty, unverifiable, or merge state cannot be proven
