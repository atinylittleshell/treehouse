...
- Every command bounds its own blocking: one process-wide budget in `internal/deadline`, armed in `rootCmd.PersistentPreRun` (`--timeout`, then `TREEHOUSE_TIMEOUT`, then `deadline.Default`; `0` waits forever). The package doc carries the design and the reasons; three decisions are not derivable from the code. It is process-wide rather than a `context.Context` through `vcs.Backend` because one process runs one command and the interface has 19 operations with three context consumers. It is DEFAULT-ON, because a bound that ships off leaves the hang in place. And it is per PHASE: `shell.Spawn`, `hooks.Run`, and `ui.Confirm` are deliberately unbounded and each calls `deadline.Restart()` on the way out, so their time is not charged to whatever runs next - a rule that lives in those three functions, never at their call sites. Any new unbounded primitive joins that list. A cancelled `git`/`jj` command kills its DESCENDANTS first (`process.KillDescendants`, enumerated before the parent dies because afterwards they are reparented), then the command: git runs its transports as separate processes that inherit the pipes. Deliberately not a process group - that would put git in the terminal's BACKGROUND group, where its credential prompt's `/dev/tty` read raises SIGTTIN and stops it.
